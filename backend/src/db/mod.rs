...
async fn init_mysql_schema(pool: &DbPool) -> Result<(), sqlx::Error> {
    let statements = [
        ...
        CREATE TABLE IF NOT EXISTS subscriptions (
          ...
          UNIQUE KEY idx_subscriptions_portal_slug (portal_slug),
...
async fn apply_mysql_schema_upgrades(pool: &DbPool) -> Result<(), sqlx::Error> {
    ...
    for (table, column, definition) in [
        ...
        ("subscriptions", "portal_slug", "VARCHAR(191) NULL"),
        (
            "subscriptions",
        ...
        ("node_ip_probes", "risk_ip", "VARCHAR(64) NULL"),
        ("node_ip_probes", "risk_status", "VARCHAR(64) NULL"),
        ("node_ip_probes", "scamalytics_fraud_score", "BIGINT NULL"),
        (
            "node_ip_probes",
            "scamalytics_isp_risk_score",
            "BIGINT NULL",
        ),
        ("node_ip_probes", "risk_checked_at", "VARCHAR(64) NULL"),
        ("node_ip_probes", "risk_expires_at_unix_ms", "BIGINT NULL"),
        ("node_ip_probes", "risk_message", "TEXT NULL"),
        ("node_ip_probes", "risk_traits_json", "TEXT NULL"),
        (
            "node_ip_probes",
    ...
    for (table, index, create_sql) in [
        ...
        (
            "subscriptions",
            "idx_subscriptions_portal_slug",
            "CREATE UNIQUE INDEX idx_subscriptions_portal_slug ON subscriptions(portal_slug)",
        ),
        (
            "upstream_subscriptions",
        ...
        (
            "node_ip_probes",
            "idx_node_ip_probes_risk_status",
            "CREATE INDEX idx_node_ip_probes_risk_status ON node_ip_probes(risk_status)",
        ),
    ] {
