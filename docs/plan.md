...
- 2026-08-20 订阅稳定性修复：节点删除前新增订阅引用检查，已被普通订阅使用的节点不再允许硬删除，避免 `subscription_nodes` 级联删除导致普通订阅链接变为空配置；删除上游订阅并选择“同时删除相关节点”时也会阻止删除仍被普通订阅引用的节点。
- 2026-08-20 前端保护：节点删除和批量删除确认文案明确说明订阅引用保护；订阅 token 轮换增加二次确认并提示旧链接立即失效；复制/二维码默认订阅链接改为固定携带订阅默认客户端 `target`，避免不同客户端 User-Agent 推断不一致。
- 2026-08-20 按组跟随订阅：新增 `subscription_node_groups` 关联，普通订阅可选择自动跟随一个或多个节点分组；上游同步后新增节点自动进入订阅链接、同名参数变化继续原地更新、上游缺失或暂停节点在导出时自动排除，且同步后立即清理公开导出缓存。
- 2026-08-20 管理端增强：订阅编辑器新增“自动跟随节点分组”选择区，当前分组可一键转为跟随模式，列表和详情展示跟随分组数量；原有订阅默认仍保持固定节点选择，不会被迁移静默改为全组跟随。
- 2026-08-20 发布准备：后端、前端和锁文件版本号更新到 `0.1.7`，`CHANGELOG.md` 新增中文 `v0.1.7` 更新日志，覆盖订阅链接稳定性、按分组跟随上游同步、暂停节点导出过滤和删除引用保护。
